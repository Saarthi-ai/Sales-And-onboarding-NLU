...
def get_abs_path(rel_path):
    ...
    """
    path = os.getenv("AZURE_SOURCE_DIRECTORY")
    deployed_in_container = os.path.isdir("/var/azureml-app/")
    
    if deployed_in_container:
        if path is None:
            path = "/var/azureml-app/saarthi-ner"
            logging.warning(f"Environment variable AZURE_SOURCE_DIRECTORY not set, but /var/azureml-app/ exists, defaulting to {path}")
        
        folder_name = os.path.basename(os.path.dirname(__file__))
        data_file_path = os.path.join(path, f"src/ner/postprocessing/inverse_text_normalization/{folder_name}", rel_path)
