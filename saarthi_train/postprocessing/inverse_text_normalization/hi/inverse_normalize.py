...
# from saarthi_train.postprocessing.inverse_text_normalization.lang_params import LANG
# lang_taggers = f'saarthi_train.postprocessing.inverse_text_normalization.taggers.{LANG}_taggers'
# lang_taggers = 'saarthi_train.postprocessing.inverse_text_normalization.hi.taggers.hi_taggers'

# exec(f"from {lang_taggers}.tokenize_and_classify_final import ClassifyFinalFst")

from saarthi_train.postprocessing.inverse_text_normalization.hi.taggers.tokenize_and_classify_final import (
    ClassifyFinalFst,
)
from saarthi_train.postprocessing.inverse_text_normalization.hi.token_parser import (
    PRESERVE_ORDER_KEY,
    TokenParser,
)
from saarthi_train.postprocessing.inverse_text_normalization.hi.verbalizers.verbalize_final import (
    VerbalizeFinalFst,
)
...
try:
    ...
    tgvp_file = get_abs_path("tagger_verbalizer_parser.pkl")

    recompile_fsts = not os.path.isfile(tgvp_file)
    if not recompile_fsts:
        with open(tgvp_file, "rb") as f:
            try:
                tagger, verbalizer, parser = pickle.load(f)
                # logger.info("loading taggger verbalizer parser from .pkl")
            except ModuleNotFoundError:
                # logger.warn(
                #     "Directory structure of python package probably changed.\
                #     Would need to rebuild tagger, verbalizer, parser. Deploy again!"
                # )
                recompile_fsts = True
    print("recompile_fsts:", recompile_fsts)
    if recompile_fsts:
        # logger.warn("Compiling taggger verbalizer parser")
        tagger = ClassifyFinalFst()
        verbalizer = VerbalizeFinalFst()
        parser = TokenParser()
        logger.info(f"Saving models at: {tgvp_file}")
        with open(tgvp_file, "wb") as f:
            pickle.dump((tagger, verbalizer, parser), f)
    ...
    for perm in d_permutations:
        ...
        for k, v in perm:
            if isinstance(v, str):
                ...
                subl = [
                    "".join(x)
                    for x in itertools.product(subl, [f" {k} {{ "], rec, [f" }} "])
                ]
...
def inverse_normalize(text: str, verbose: bool) -> str:
    ...
    tagged_text = re.sub("ordinal", "cardinal", tagged_text)
    parser(tagged_text)
    tokens = parser.parse()
